...
**[money-flow-map.html](money-flow-map.html)** — 出入金实测链路数据稿，一链路一行，**Bloomberg 终端风**。共 **38 条链路 / 112 个节点**，按验证状态分四节、行首带连续编号：

| 节 | 条数 | 含义 |
|---|---|---|
| VERIFIED | 34 | 已实测闭环（01–34） |
| PARTIAL | 2 | 部分闭环 / 片段（35–36） |
| PENDING | 2 | 待闭环（37–38） |
| NOT TESTED | 0 | 未实测（当前无） |

**VERIFIED 头部两条**为用户实测落地的稳定币链路（Arbitrum One / USDT0 → Bybit、N26 EUR → Neverless → USDT0 → ether.fi → OP → Bybit），tag「中转」。节点类型涵盖支付、卡产品、金融机构、银行、交易所、公链、经纪商等。
